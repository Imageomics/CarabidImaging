...
Small<-read.csv("./BeetleMetadataSmallBeetles.csv")
Small$imageSource<-"SmallBeetles Imaging"
...
bit::symdiff(names(Small), names(ABTrays))

common_cols <- intersect(names(ABTrays), names(Small))
length(common_cols)
...
table(duplicated(ImageMetadata$imageID))

#doubleImaged<-ImageMetadata[duplicated(ImageMetadata$imageID), ]
