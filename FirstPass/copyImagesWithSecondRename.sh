...
# Define paths
CSV="$WORKDIR/catalog_firstPass_renamedMetadataClean.csv"
SRC_DIR="$WORKDIR/Images/FirstPassClean"
...
tail -n +2 "$CSV" | awk -F',' '{print $21","$1}' | while IFS=',' read -r imageID newImageID; do
