...
firstpass_df <- as.data.frame(read_excel("./BeetleMetadata.xlsx", sheet = 3))
...
#First We need to handle to images with a single beetle in them
for (i in 1:nrow(firstpass_df)) {
  ...
  if (row$NumberOfBeetles == 1) {
    ...
    if (inImageQuery$scientificName_Species == row$scientificName_Species) {
...
write.csv(firstpass_out_df, paste0("./BeetleMetadata",dataset,".csv"), row.names = FALSE)
...
for (i in 1:nrow(df_remainingmissmatch)) {
  ...
  if (num_found > row$NumberOfBeetles) {
    ...
    outfile <- paste0("./NEONIndividualLinkageChecks/SmallBeetles/CHECK_",
...
# Define directories
csv_dir <- "/fs/ess/PAS2136/CarabidImaging/NEONIndividualLinkageChecks/SmallBeetles"
