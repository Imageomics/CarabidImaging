...
def detect_scalebar(image, model):
    """Run detection on the image and return the detected scalebar objects."""
    width, height = image.size
    result = model.detect(image, "a scalebar")
...
def main(image_dir, output_dir, output_csv):
    ...
    for image_path in image_paths:
        try:
            ...
            coords_list = detect_scalebar(image, model)
            ...
            row = {
                ...
                "number of scalebars": len(coords_list),
                "scalebar coordinates": str(coords_list),
    ...
    with open(output_csv, "w", newline="") as csvfile:
        fieldnames = ["image_name", "plotted_image", "number of scalebars", "scalebar coordinates", "checkflag"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scalebar Detection in Tray Images")
    parser.add_argument(
        "--image_dir",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/fs/ess/PAS2136/CarabidImaging-sample-output/TestOutputABTraysScalebar",
        help="Directory to save plotted output images."
    )
    parser.add_argument(
        "--output_csv",
        type=str,
        default="/fs/ess/PAS2136/CarabidImaging-sample-output/TestOutputABTraysScalebar/scalebars.csv",
